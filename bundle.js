...
const synthapp = (type, expr) => TC_1.log(`synthapp ${type} @ ${expr}`).chain(() => {
    ...
    if (types_1.isTMeta(type))
        return TC_1.findTMeta(type.name)
            ...
            elems_1.ctmeta(a2, kinds_1.kType), elems_1.ctmeta(a3, kinds_1.kEffs), elems_1.ctmeta(a1, kinds_1.kType), e.solve(types_1.tfun(types_1.tmeta(a1), types_1.tcomp(types_1.tmeta(a2), types_1.tmeta(a3))))
        ])
            .then(checkVal(expr, types_1.tmeta(a1))
            .map(() => types_1.tcomp(types_1.tmeta(a2), types_1.tmeta(a3))))));
    if (types_1.isTFun(type))
