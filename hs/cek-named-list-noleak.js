...
const makeEnv = (env, fr, had) => {
  if (env.tag === NIL) return env;
  const { head: { key, value }, tail } = env;
  if (had[key] || !fr[key]) return makeEnv(tail, fr, had);
  had[key] = true;
  return Cons(env.head, makeEnv(tail, fr, had));
};
...
const step = state => {
  ...
  if (cont.tag === FUN) {
    const fn = cont.abs;
    state.term = fn.body;
    state.env = Cons(KV(fn.name, makeClos(term, env)), cont.env);
