...
export default class Map<K extends Hash, V> {
  imap: InternalMap<K, V>;
  constructor(map: InternalMap<K, V>) {
    this.imap = map;
  }

  static from<K extends Hash, V>(pairs: KVPair<K, V>[]) {
    let map: InternalMap<K, V> = {};
    for(let i = 0, l = pairs.length; i < l; i++)
      map[pairs[i][0].hash()] = pairs[i][1];
    return new Map<K, V>(map);
  }
  static of<K extends Hash, V>(...pairs: KVPair<K, V>[]) {
    return Map.from(pairs);
  }
  static empty<K extends Hash, V>(): Map<K, V> { return Map.of<K, V>() }
